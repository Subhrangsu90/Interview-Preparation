...
(function () {
  ...
  function getIcon(name, ext, isDir) {
    if (isDir) {
      const dirIcons = { "JavaScript": "💛", "TypeScripts": "🔷", "Basics": "📗", "Intermediate": "📙", "Advanced": "📕" };
      return dirIcons[name] || "📁";
    }
  ...
  function renderTree() {
    ...
    dom.sidebarStats.innerHTML = `
      <span class="stat-badge">📁 ${folderCount} folders</span>
  ...
  async function loadFile(filePath) {
    ...
    if (IMAGE_EXTS.includes(ext)) {
      ...
      dom.tocContainer.style.display = "none";
      return;
    }
  ...
  function renderMarkdown(content, filePath, fileName) {
    ...
    renderer.code = function (codeStr, lang, escaped) {
      ...
      return `<pre><div class="code-block-header"><span class="code-lang-tag">${lang || "code"}</span><button class="code-copy-btn" onclick="copyCode(this)">📋 Copy</button></div><code class="${langClass}">${highlighted}</code></pre>`;
    };
      ...
      <div class="vscode-container">
        ...
        <div class="vscode-topbar">
          ...
          <div class="vscode-actions">
            ${['.js', '.ts'].includes(ext) ? `<button class="vscode-btn vscode-btn-run" onclick="window.__runJSCode()" title="Run Code (Ctrl+Enter)">▶ Run Code</button>` : ''}
            <button class="vscode-btn vscode-btn-save" onclick="window.__saveCodeFile()" title="Save File (Ctrl+S)">💾 Save</button>
            <button class="vscode-btn" onclick="window.__formatCodeFile()" title="Format Code">🎨 Format</button>
            <button class="vscode-btn" onclick="window.__copyCodeFile()" title="Copy Code">📋 Copy</button>
          </div>
        </div>
        ...
        <div class="vscode-console-panel" id="vscode-console-panel" style="display:none;">
          <div class="vscode-console-header">
            <span class="vscode-console-title">🖥️ Console</span>
            <span class="vscode-console-meta" id="vscode-console-meta"></span>
            <button class="vscode-console-clear" onclick="window.__clearConsole()" title="Clear Console">🚫 Clear</button>
            <button class="vscode-console-close" onclick="document.getElementById('vscode-console-panel').style.display='none'">✕</button>
          </div>
  ...
  window.__runJSCode = function () {
    ...
    const customConsole = {
      ...
      warn: (...args) => logs.push({ type: "warn", html: `<span class="dev-icon">⚠️</span> ` + formatConsoleItem(args), indent: getIndentPx() }),
      error: (...args) => logs.push({ type: "error", html: `<span class="dev-icon">❌</span> ` + formatConsoleItem(args), indent: getIndentPx() }),
      info: (...args) => logs.push({ type: "info", html: `<span class="dev-icon">ℹ️</span> ` + formatConsoleItem(args), indent: getIndentPx() }),
      ...
      timeEnd: (label = "default") => {
        if (timers[label]) {
          ...
          logs.push({ type: "info", html: `<span class="dev-icon">⏱️</span> <span>${escapeHtml(label)}: ${elapsed}ms</span>`, indent: getIndentPx() });
          delete timers[label];
        }
      },
      count: (label = "default") => {
      ...
      assert: (cond, ...args) => {
        if (!cond) {
          ...
          logs.push({ type: "error", html: `<span class="dev-icon">❌</span> Assertion failed: ${escapeHtml(msg)}`, indent: getIndentPx() });
        }
      },
      trace: (...args) => {
        const msg = args.map(formatConsoleArg).join(" ");
        const stack = new Error().stack.split("\n").slice(2).join("\n");
        logs.push({ type: "warn", html: `<span class="dev-icon">⚠️</span> Trace: ${escapeHtml(msg)}<pre class="console-trace">${escapeHtml(stack)}</pre>`, indent: getIndentPx() });
      },
      clear: () => {
        logs.length = 0;
        consoleBody.innerHTML = "";
    ...
    try {
      ...
      if (logs.length === 0) {
        consoleBody.innerHTML = `<div class="console-line console-meta">⚡ Code executed with no console output.</div>`;
      } else {
  ...
  window.__evalConsolePrompt = function (expr) {
    ...
    promptLine.innerHTML = `<span class="dev-icon">›</span> <code>${escapeHtml(expr)}</code>`;
    ...
    try {
      ...
      resultLine.innerHTML = `<span class="dev-icon">‹</span> ${formatConsoleItem([res])}`;
      consoleBody.appendChild(resultLine);
    } catch (err) {
      const errorLine = document.createElement("div");
      errorLine.className = "console-line console-error";
      errorLine.innerHTML = `<span class="dev-icon">❌</span> Uncaught ${escapeHtml(err.message)}`;
    ...
    dom.contentInner.innerHTML = `
      <div class="image-viewer">
        <div class="code-file-header">
          <span class="code-file-icon">🖼️</span>
  ...
  function updateBreadcrumb(filePath) {
    ...
    let html = `<span class="breadcrumb-item" onclick="window.__goHome()">📁 Interview Preparation</span>`;
  ...
  window.__goHome = function () {
    ...
    if (dom.welcomeScreen) {
      dom.contentInner.appendChild(dom.welcomeScreen);
      dom.welcomeScreen.style.display = "";
    } else {
      dom.contentInner.innerHTML = `
        <div class="welcome-screen">
          <div class="welcome-icon">✏️</div>
          <h2 class="welcome-title">Welcome to your Notes</h2>
          <p class="welcome-subtitle">Pick a file from the sidebar to start reading.</p>
          <div class="welcome-hint">💡 New files you create will appear automatically!</div>
        </div>
      `;
    }
    dom.breadcrumb.innerHTML = `<span class="breadcrumb-item">📁 Interview Preparation</span>`;
    dom.tocContainer.style.display = "none";
    $$(".tree-item-row.active").forEach((r) => r.classList.remove("active"));
  };
  ...
  // ═══════════════════════════════════
  function buildTOC() {
    const headings = dom.contentInner.querySelectorAll("h1, h2, h3, h4");
    ...
    headings.forEach((h, i) => {
      ...
      const id = "heading-" + i;
      h.id = id;
      html += `<a href="#${id}" class="toc-link depth-${level}" onclick="event.preventDefault(); document.getElementById('${id}').scrollIntoView({behavior:'smooth', block:'start'})">${h.textContent}</a>`;
    });
    ...
    evtSource.onmessage = (event) => {
      ...
      try {
        ...
        if (state.currentFile && change.path === state.currentFile && change.event === "change") {
          ...
          showToast("🔄", `Updated: ${change.path.split("/").pop()}`);
        }

        // Notify on new files
        if (change.event === "add") {
          showToast("✨", `New file: ${change.path.split("/").pop()}`);
        }
  ...
  window.copyCode = function (btn, isFullFile) {
    ...
    navigator.clipboard.writeText(code).then(() => {
      ...
      setTimeout(() => {
        btn.textContent = isFullFile ? "📋 Copy All" : "Copy";
  ...
  function init() {
    ...
    // Apply lines
    if (!state.showLines) dom.contentArea.classList.add("no-lines");
    ...
    $("#toggle-lines").addEventListener("click", () => {
      ...
      dom.contentArea.classList.toggle("no-lines", !state.showLines);
      localStorage.setItem("ip-lines", state.showLines);
      $("#toggle-lines").classList.toggle("active", state.showLines);
    });
